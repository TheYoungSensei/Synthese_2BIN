...
public class Controleur extends javax.servlet.http.HttpServlet implements
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			Propriete laClasse = new Propriete(nomClasse,
			...
			for (Field field : cls.getDeclaredFields()) {			
				...
				if (Modifier.isStatic(field.getModifiers()))
					estStatique = true;
				String valDefaut="";
				Propriete prop = new Propriete(field.getName() + " : "
						+ field.getType().getSimpleName()+valDefaut, estStatique,
						visibilite);
				attributs.add(prop);
			}
	...
	private List<Propriete> chercherConstruteurs(Class cls) {
		...
		for (Constructor constructeur : cls.getDeclaredConstructors()) {			
			Visibilite visibilite = chercherVisibilite(constructeur
					.getModifiers());
			boolean estStatique = false;
			...
			Propriete prop = new Propriete(nom, estStatique,
					Modifier.isAbstract(constructeur.getModifiers()),
					visibilite);
	...
	private List<Propriete> chercherMethodes(Class cls) {
		...
		for (Method method : cls.getDeclaredMethods()) {				
			...
			Propriete prop = new Propriete(nom + " : "
					+ method.getReturnType().getSimpleName(), estStatique,
					Modifier.isAbstract(method.getModifiers()), visibilite);
			methodes.add(prop);
		}
		return methodes;
	}
	...
	/*
	 * Méthode utilitaire qui a analyse l'en-tête "content-disposition" et
	 * vérifie si le paramètre "filename" y est présent. Si oui, alors le champ
	 * traité est de type File et la méthode retourne son nom, sinon il s'agit
	 * d'un champ de formulaire classique et la méthode retourne null.
	 */
	...
	private void ecrireFichier(Part part, String nomFichier, String chemin)
			throws IOException {
		try (BufferedInputStream entree = new BufferedInputStream(
				part.getInputStream(), TAILLE_TAMPON);
				BufferedOutputStream sortie = new BufferedOutputStream(
						new FileOutputStream(new File(chemin + nomFichier)),
						TAILLE_TAMPON)) {
