...
public class Personne {
	...
	/**
	 * Lance un thread vendeur pour un lot. Celui-ci va diminuer progressivement
	 * le prix du lot jusqu'à atteindre le prixPlancher.
	 * 
	 * @param lot Lot à vendre. Il doit appartenir à la personne
	 * @param prixInitial
	 * @param montantDiminution
	 *            Diminution du prix à chaque intervalle
	 * @param prixPlancher
	 * @param intervalleTemps
	 *            Intervalle de décrémentation du prix
	 * @return Le thread vendeur si on a pu le créer
	 * @throws LotDejaEnVenteException
	 *             si le lot est déjà en vente
	 * @throws NullPointerException
	 *             si le lot est null
	 * @throws ThisIsNotMyBatchException
	 *             si le lot ne nous appartient pas
	 */
	public Thread mettreUnLotEnVente(Lot lot, double prixInitial,
			double montantDiminution, double prixPlancher, int intervalleTemps) throws LotDejaEnVenteException, ThisIsNotMyBatchException {
		Vendeur vendeur = new Vendeur(lot, prixInitial, montantDiminution, prixPlancher, intervalleTemps);
	...
	private class Vendeur implements Runnable{
		...
		public Vendeur(Lot lotEnVente, double prixInitial, double montantDiminution, double prixPlancher,
				int intervalleTemps) throws ThisIsNotMyBatchException, LotDejaEnVenteException {
			checkObject(lotEnVente);
			checkPositive(intervalleTemps);
			checkPositive(montantDiminution);
			checkPositive(prixPlancher);
			if (prixInitial < prixPlancher) throw new IllegalArgumentException();
			synchronized (lotEnVente){
				if (lotEnVente.getProprietaire() != Personne.this) throw new ThisIsNotMyBatchException();
				if (lotEnVente.isEnVente()) throw new LotDejaEnVenteException();
		...
		public void run() {
			...
			System.out.println(
					"Mise en vente du lot numéro " + lotEnVente.getNumero() + " au prix initial de " + prixInitial);
			do {
				try {
				...
				synchronized (lotEnVente) {
					if (lotEnVente.isEnVente() && lotEnVente.getProprietaire() == Personne.this
						...
						System.out.println("Diminution du prix du lot numéro " + lotEnVente.getNumero() + " au prix de "
								+ lotEnVente.getPrixUnitaire());
					} else {
						break;
					}
				}
				...
				while (lotsEnVente.hasNext() && !Thread.currentThread().isInterrupted()) {
					...
					synchronized (lot) {
						if (lot.isEnVente() && lot.getProprietaire() != Personne.this
								&& lot.getQuantite() <= quantiteRestante && lot.getPrixUnitaire() <= prixUnitaireMax) {
							acheterLot(lot);
