...
public class Exercices5Etudiants {
  ...
  public static void main(String[] args) {
    ...
    try (Stream<String> lignes = lignesSupplier.get()) {
      System.out.println("\nEx2a\n====");
      List<Etudiant> lS = lignes.map(s -> s.split(";")).map(tabS -> biFunct.apply(tabS[3], tabS[0]))
    ...
    try (Stream<String> lignes = lignesSupplier.get()) {
      System.out.println("\nEx2b\n====");
      List<Etudiant> lS2 = lignes.map(s -> funct.apply(s)).collect(Collectors.toList());
    } catch (UncheckedIOException e) {
      e.printStackTrace();
    }
    ...
    try (Stream<String> lignes = lignesSupplier.get()) {
      System.out.println("\nEx3\n====");
      List<Etudiant> lS3 = lignes.map(s -> funct.apply(s)).collect(Collectors.toList());
    ...
    try (Stream<String> lignes = lignesSupplier.get()) {
      System.out.println("\nEx4\n====");
      String aeNom = lignes.map(s -> funct.apply(s)).map(Etudiant::getPrenom)
          .filter(Exercices5Etudiants::isAeWord).findAny().orElse("none");
    } catch (UncheckedIOException e) {
      e.printStackTrace();
    }
    ...
    try (Stream<String> lignes = lignesSupplier.get()) {
      ...
      System.out.println(lignes.map(s -> funct.apply(s)).filter(pred1).map(Etudiant::getMatricule)
          .findAny().orElse("none"));
    } catch (UncheckedIOException e) {
      e.printStackTrace();
    }
