...
var joueur = batailleNavale();
placerBateauxAleatoire(joueur);
var ordi = batailleNavale();
placerBateauxAleatoire(ordi);

function joue(coord) {
    var notif = document.getElementById("notif");
    var divsJoueur = document.getElementsByClassName("player");
    var divsOrdi = document.getElementsByClassName("adv");
    var test = document.getElementById("test");
    var coup = joueur.tirer(ordi, getCoordA(coord), getCoordB(coord));
    if (!checkSiFinis(ordi) || !checkSiFinis(joueur)) {
        if (coup === "touché") {
            divsOrdi[(getCoordB(coord) * 10) + getCoordA(coord) - 1].innerHTML = "";
            divsOrdi[(getCoordB(coord) * 10) + getCoordA(coord) - 1].style.backgroundColor = "red";
        } else if (coup === "A l'eau") {
            divsOrdi[(getCoordB(coord) * 10) + getCoordA(coord) - 1].innerHTML = "";
            divsOrdi[(getCoordB(coord) * 10) + getCoordA(coord) - 1].style.backgroundColor = "blue";
        }
        notif.innerHTML = "Au tour de l'ordi";
        if (!checkSiFinis(ordi)) {
            var coordA = unEntierAuHasardEntre(1, 10);
            var coordB = unEntierAuHasardEntre(1, 10);
            coup = ordi.tirer(joueur, coordA, coordB);
            if (coup == "touché") {
                divsJoueur[coordB * 10 + coordA - 1].style.backgroundColor = "red";
            } else if (coup == "A l'eau") {
                divsJoueur[coordB * 10 + coordA - 1].style.backgroundColor = "blue";
            }
        }
    }
    if(checkSiFinis(ordi)) {
        notif.innerHTML = "Le joueur domine les mers";
    } else if(checkSiFinis(joueur)){
        notif.innerHTML = "L'ordi domine les mers";
    }
}
