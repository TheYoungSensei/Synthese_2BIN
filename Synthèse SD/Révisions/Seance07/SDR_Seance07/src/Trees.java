...
public class Trees {
  ...
  public static void printPathV3(Tree t, int v) {
    if (t.getParent() == null)
      ...
      printPathV3(t.getParent(), v);
  }

  private static void findTree(Tree t, int v) {
    if (t.getValue() == v)
      printPathV1(t);
    else {
      for (Tree tree : t.getChildren()) {
  ...
  public static int[][] toArray(Tree t) {
    return toArrayPrivate(t, null);
  }

  private static int index;

  private static int[][] toArrayPrivate(Tree t, int[][] array) {
    if (array == null) {
